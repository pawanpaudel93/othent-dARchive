...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (result.status === "success") {
      ...
      await Promise.all(
        files
          ...
          .map(async (file) => {
            ...
            const {data, tags} = await prepareFile(filePath, result.title, url, result.timestamp, isIndexFile)
      ...
      const transactionId = await uploadToBundlr(manifestData, manifestTags, accessToken)
